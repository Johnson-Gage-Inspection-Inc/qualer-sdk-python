...
class QualerApiModelsMeasurementsFromUpdateMeasurementPointModel(object):
    ...
    def tool_application_mode(self, tool_application_mode):
        ...
        if (
            self._configuration.client_side_validation
            and tool_application_mode not in allowed_values
        ):
            raise ValueError(
                "Invalid value for `tool_application_mode` ({0}), must be one of {1}".format(  # noqa: E501
                    tool_application_mode, allowed_values
                )
            )
    ...
    def __eq__(self, other):
        ...
        if not isinstance(
            other, QualerApiModelsMeasurementsFromUpdateMeasurementPointModel
        ):
            return False
