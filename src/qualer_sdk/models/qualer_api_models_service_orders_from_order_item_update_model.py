...
class QualerApiModelsServiceOrdersFromOrderItemUpdateModel(object):
    ...
    def as_found_check(self, as_found_check):
        ...
        if (
            self._configuration.client_side_validation
            and as_found_check not in allowed_values
        ):
            raise ValueError(
                "Invalid value for `as_found_check` ({0}), must be one of {1}".format(  # noqa: E501
                    as_found_check, allowed_values
                )
            )
    ...
    def as_left_check(self, as_left_check):
        ...
        if (
            self._configuration.client_side_validation
            and as_left_check not in allowed_values
        ):
            raise ValueError(
                "Invalid value for `as_left_check` ({0}), must be one of {1}".format(  # noqa: E501
                    as_left_check, allowed_values
                )
            )
